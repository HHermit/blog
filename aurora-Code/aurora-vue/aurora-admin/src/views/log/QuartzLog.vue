<template>
  <el-card class="main-card">
    <el-form :inline="true" ref="form" label-width="68px">
      <el-row>
        <el-form-item label="任务名称">
          <el-input style="width: 200px" size="small" v-model="searchParams.jobName" placeholder="请输入任务名称"
            @keyup.enter.native="listJobLogs" />
        </el-form-item>
        <el-form-item label="任务组名">
          <el-select size="small" @change="listJobLogs" clearable style="margin-left: 5px" v-model="searchParams.jobGroup"
            placeholder="请选择组名">
            <el-option v-for="jobGroup in jobGroups" :key="jobGroup" :label="jobGroup" :value="jobGroup" />
          </el-select>
        </el-form-item>
        <el-form-item label="执行状态">
          <el-select size="small" @change="listJobLogs" clearable style="margin-left: 5px" v-model="searchParams.status"
            placeholder="请选择任务状态">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button :disabled="!jobLogIds.length" type="danger" icon="el-icon-delete" size="small"
          @click="deleteJobLogs">批量删除
        </el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table :default-sort="{ prop: 'createTime', order: 'descending' }" style="width: 100%" :data="jobLogs"
    ...
    <el-dialog title="调度日志详细" :visible.sync="open" :width="jobLog.status == 1 ? '700px' : '80%'" append-to-body
      destroy-on-close>
