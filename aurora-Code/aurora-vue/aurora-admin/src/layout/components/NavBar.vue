...
export default {
  created() {
    // 获取当前路由的匹配项：通过 filter 方法过滤出 name 属性不为空的路由。以内当前的路由中，不带name，而他的孩子带name。
    //单级的父级菜单
    let matched = this.$route.matched.filter((item) => item.name)
    //获取第一个匹配项，并判断其名称是否为“首页”。如果不是，则在匹配项数组的前面插入一个名为“首页”的路由记录。
    const first = matched[0]
    if (first && first.name !== '首页') {
      matched = [{ path: '/', name: '首页' }].concat(matched)
    }
    //将处理后的路由记录赋值给组件的 breadcrumbs 属性，用于展示面包屑导航。
