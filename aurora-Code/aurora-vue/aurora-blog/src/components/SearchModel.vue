<template>
  <div v-if="openModal" id="search-modal" @keydown.esc="handleStatusChange(false)"
    @keydown.meta.k.stop.prevent="handleStatusChange(false)" @keydown.arrow-up.stop.prevent="handleArrowUp"
    @keydown.arrow-down.stop.prevent="handleArrowDown" @keydown.enter.stop.prevent="handleEnterDown"
    @click.self="handleStatusChange(false)" tabindex="-1">
        ...
        <header class="flex pt-4 pr-4 pl-4">
          <form class="search-form" action="">
            <label id="search-label" class="items-center flex justify-center" for="search-input">
              <svg class="text-ob fill-current stroke-current" width="32" height="32" viewBox="0 0 24 24" fill="none"
                xmlns="http://www.w3.org/2000/svg" data-reactroot="">
                <path stroke-linejoin="round" stroke-linecap="round" stroke-width="1" stroke=""
                  d="M15.9996 15.2877L15.2925 15.9948L21.2958 21.9981L22.0029 21.291L15.9996 15.2877Z"></path>
                <path stroke-linejoin="round" stroke-linecap="round" stroke-width="1" stroke="" fill="rgba(0,0,0,0)"
                  d="M10 18C14.4183 18 18 14.4183 18 10C18 5.58172 14.4183 2 10 2C5.58172 2 2 5.58172 2 10C2 14.4183 5.58172 18 10 18Z">
                </path>
              </svg>
            </label>
            <input type="search" id="search-input" ref="searchInput" class="search-input" autocomplete="off"
              v-model="keywords" @input="searchKeywords" />
            <button v-show="keywords.length > 0" class="search-btn" type="reset" title="Clear the query"
        ...
        <div id="Search-Dropdown" class="search-dropdown" v-if="searchResults !== null">
          <div>
            <section v-if="searchResults.length > 0">
              ...
              <ul id="search-menu">
                <li v-for="(result, index) in searchResults" :key="result.id" :class="{
                  'search-hit': true,
            ...
            <section v-else>
              ...
              <ul id="search-menu">
                <li v-for="(result, index) in recentResults" :key="result.id" :class="{
                  'search-hit': true,
                  active: index == menuActiveIndex
                }" :id="'search-hit-item-' + index">
                  <a href="javascript:void(0)" @click="handleLinkClick(result)">
        ...
        <div class="search-footer">
          <div class="search-logo">
            <a href="" target="_blank" rel="noopener noreferrer">
              ...
              <!-- <img class="mr-1.5" src="https://static.linhaojun.top/aurora/config/cc36e9fa5aeb214e41495c1e2268f2db.png"
                alt="ObsidianNext Logo" height="20" width="20" /> -->
              <svg-icon icon-class="search-icon" class="mr-1.5" alt="ObsidianNext Logo" height="20" width="20" />
