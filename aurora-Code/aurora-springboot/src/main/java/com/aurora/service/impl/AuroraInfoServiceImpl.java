...
public class AuroraInfoServiceImpl implements AuroraInfoService {
    ...
    public void report() {
        ...
        //根据用户ip 浏览器 操作系统生成唯一标识 存储在 redis 中  记录当前访客
    ...
    public AuroraAdminInfoDTO getAuroraAdminInfo() {
        ...
        //从Redis服务中获取ARTICLE_VIEWS_COUNT有序集合中排名前5的文章ID及其对应的浏览次数（分数），并将其存储在一个Map对象中，其中文章ID作为键，浏览次数作为值。
        ...
        AuroraAdminInfoDTO auroraAdminInfoDTO = AuroraAdminInfoDTO.builder()
                ...
                .build();
        //将articleMap封装为ArticleRankDTO形式并返回
    ...
    public WebsiteConfigDTO getWebsiteConfig() {
        ...
        //从Redis中获取配置信息，不存在就从数据库中获取，并将配置信息存储到Redis中
    ...
    /**
    * @Description: 根据排行中的表格信息封装为ArticleRankDTO对象
    * @Param: [articleMap]
    * @return: java.util.List<com.aurora.model.dto.ArticleRankDTO>
    */
